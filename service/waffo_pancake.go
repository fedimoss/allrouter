...
type WaffoPancakeCreateSessionParams struct {
	...
	// OrderMerchantExternalID is echoed by Pancake as
	// data.orderMerchantExternalId in webhook payloads. Pancake's orderId is
	// gateway-generated, so the local trade number must be carried explicitly
	// to make callback resolution reliable.
...
type WaffoPancakeWebhookData struct {
	OrderID string
	// OrderMerchantExternalID is the merchant-owned reference echoed by
	// Pancake. It is distinct from OrderID, which is generated by Pancake.
	OrderMerchantExternalID       string
...
func CreateWaffoPancakeCheckoutSession(ctx context.Context, params *WaffoPancakeCreateSessionParams) (*WaffoPancakeCheckoutSession, error) {
	...
	// SDK v0.2.x does not expose OrderMerchantExternalID yet, but it does
	// preserve checkout Metadata in webhook data.orderMetadata.  Carry the
	// local trade number there so callbacks can be resolved independently of
	// Pancake's gateway-generated orderId.  Newer SDKs can map the same value to
	// their dedicated external-id field without changing this service API.
	metadata := map[string]string{}
	if reference := strings.TrimSpace(params.OrderMerchantExternalID); reference != "" {
		metadata["new_api_trade_no"] = reference
	}
	// Echo the immutable product id in order metadata.  The webhook payload
	// does not expose a product-id field in the current SDK, so this metadata is
	// the only reliable way to prove that a signed completion belongs to the
	// exact subscription product selected at checkout (rather than another
	// product with the same amount/currency).
	if productID := strings.TrimSpace(params.ProductID); productID != "" {
		metadata["new_api_product_id"] = productID
	}
...
func VerifyConfiguredWaffoPancakeWebhook(payload string, signatureHeader string) (*WaffoPancakeWebhookEvent, error) {
	...
	return &WaffoPancakeWebhookEvent{
		...
		Data: WaffoPancakeWebhookData{
			...
			OrderMerchantExternalID:       merchantTradeReference(evt.Data.OrderMetadata),
...
func merchantTradeReference(metadata map[string]string) string {
	for _, key := range []string{"new_api_trade_no", "reference_id", "trade_no"} {
...
func ResolveWaffoPancakeTradeNo(event *WaffoPancakeWebhookEvent) (string, error) {
	...
	// Prefer the merchant-owned external reference. `OrderID` is a Pancake
	// generated identifier and is retained only as a legacy fallback.
