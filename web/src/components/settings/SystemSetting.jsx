...
const SystemSetting = () => {
    ...
    if (success) {
      ...
      data.forEach((item) => {
        switch (item.key) {
          ...
          case 'TelegramWebhookSecretConfigured':
      ...
      setWebhookCpolar(
        mergedInputs.TelegramWebhookDomain || mergedInputs.ServerAddress || '',
      );
      setIsLoaded(true);
    } else {
      showError(message);
  ...
  const submitTelegramSettings = async () => {
    ...
    if ((inputs.TelegramWebhookSecret || '').trim()) {
      options.push({
        key: 'TelegramWebhookSecret',
        value: inputs.TelegramWebhookSecret.trim(),
      });
    }
    await updateOptions(options);
  };
  ...
  const normalizeTelegramWebhookDomain = (value) => {
  ...
  const callTelegramAdmin = async (endpoint, body) => {
    ...
    try {
      ...
      setWebhookResult(d);
      if (d.ok || d.success) {
        if (endpoint === '/api/telegram/admin/setWebhook') {
          const displayDomain = normalizeTelegramWebhookDomain(
            body.cpolar_domain,
          );
          setInputs((prev) => ({
            ...prev,
            TelegramWebhookDomain: displayDomain,
            TelegramWebhookSecret: '',
          ...
          setWebhookCpolar(displayDomain);
          formApiRef.current?.setValue('TelegramWebhookSecret', '');
          setWebhookSecret('');
        }
        showSuccess(
          endpoint === '/api/telegram/admin/setWebhook'
            ? t('Webhook 设置成功')
      ...
      showError(
        t('请求失败') + ': ' + (e.response?.data?.message || e.message),
      );
    } finally {
  ...
  const handleSetWebhook = () => {
    const secretToken = (webhookSecret || '').trim();
    if (!secretToken) {
      showWarning(
        t(
          'Webhook Secret 已配置时不会回显；如需重新设置 Webhook，请重新输入同一个或新的 Secret。',
        ),
      );
      return;
    }
              ...
              <Card>
                <Form.Section text={t('配置 Telegram 机器人')}>
                  <Text style={{ marginBottom: 16 }}>
                    {t(
                      '用以支持 Telegram 群聊入群欢迎、Mini App 账号绑定与通知消息发送。',
                    )}
                  </Text>
                  ...
                  >
                    {t(
                      '让 Telegram 把群消息、入群事件等实时推送到你的服务器。填好下方信息后点“设置 Webhook”。',
                    )}
                  </Text>
                  <Text
                    type='tertiary'
                  ...
                  >
                    {t(
                      'Webhook Secret 用于加密/校验 Telegram 回调接口，可以随意设置；重新设置 Webhook 时需再次输入，系统不会把已保存的敏感信息发送到前端显示。',
                    )}
                  </Text>
                  <Row
                    gutter={{ xs: 8, sm: 16, md: 24, lg: 24, xl: 24, xxl: 24 }}
                  >
                    ...
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <div className='semi-form-field'>
                        ...
                        <Input
                          placeholder={
                            inputs.TelegramWebhookSecretConfigured
                              ? t('敏感信息不会发送到前端显示')
                              : t('可随意设置一段高强度随机字符串')
                          }
                          type='password'
                  ...
                  {webhookResult && (
                    <Banner
                      ...
                      title={
                        webhookResult.ok ? t('Webhook 状态') : t('返回结果')
                      }
                  ...
                  >
                    {t(
                      '机器人在私聊窗口左下角会显示一个菜单按钮，点击可直接打开 Mini App。群聊里不需要这一项。',
                    )}
                  </Text>
                  <Banner
                    type='info'
                  ...
                  >
                    <ol style={{ margin: 0, paddingLeft: 18 }}>
                      <li>
                        {t('打开 @BotFather → 发送 /mybots → 选择你的机器人')}
                      </li>
                      <li>
                        {t('Bot Settings → Menu Button → Edit menu button')}
                      </li>
                      <li>
                        {t(
                          '发送你的 Mini App 完整链接（https://<你的域名>/telegram/miniapp）',
                        )}
                      </li>
                      <li>
                        {t(
                          '发送按钮标题（如“绑定用户”）。不需要时可发送 /empty 清除。',
                        )}
                      </li>
                    </ol>
                  </Banner>
                  ...
                  >
                    {t(
                      '注册 Mini App 后，群里的“绑定账号”按钮才能启动 Mini App（拿到 Telegram 用户身份）。',
                    )}
                  </Text>
                  <Banner
                    type='info'
                  ...
                  >
                    <ol style={{ margin: 0, paddingLeft: 18 }}>
                      ...
                      <li>
                        {t(
                          'Web App URL（页面地址）：https://<你的域名>/telegram/miniapp',
                        )}
                      </li>
                      <li>
                        {t(
                          'Short name（短名称）：如 test_allrouter（全小写，用于 t.me 链接）',
                        )}
                      </li>
                    </ol>
                  </Banner>
                  ...
                  >
                    {t(
                      '完成后把生成的链接（t.me/你的bot/短名称）填回上面的「Mini App 启动链接」，点保存即可。',
                    )}
                  </Text>
                  ...
                  >
                    {t(
                      '默认情况下，机器人在群里只能收到 @提及 和 /命令。关闭该模式后机器人才能收到普通群消息（如 @bot 回复、文本消息等）。',
                    )}
                  </Text>
                  <Banner
                    type='warning'
                  ...
                  >
                    {t(
                      '允许用户通过 Telegram 登录/注册网站。需要先配置好上方的基础配置和 Webhook，且在 @BotFather 中 /setdomain 登记网站域名。',
                    )}
                  </Text>
                  <Form.Checkbox
