...
export const formatDynamicPriceSummary = (billingExpr, t, groupRatio = 1) => {
  ...
  return (
    <>
      {hasCoeffs &&
        BILLING_PRICING_VARS.map((v) =>
          v.key in varCoeffs ? (
            <span key={v.key} style={lineStyle}>
              {`${t(v.label)} ${symbol}${(varCoeffs[v.key] * gr * rate).toFixed(
                4,
              )}${unitSuffix}`}
            </span>
          ) : null,
      ...
      {!hasCoeffs && tags.length === 0 && (
        <span style={lineStyle}>{t('动态计费')}</span>
      )}
    </>
  );
};

// 动态计费紧凑摘要（用于模型广场卡片，明细留到详情弹窗展示）
export const formatDynamicPriceCompactSummary = (billingExpr, t) => {
  const exprBody = (billingExpr || '').replace(/^v\d+:/, '');
  const tierMatches = exprBody.match(/tier\(/g) || [];
  const tierCount = tierMatches.length;
  const hasTimeCondition = /\b(?:hour|minute|weekday|month|day)\(/.test(
